...
namespace Financial_management_system_in_educational_institutions_API.Controllers
{
    public class InventariAktualController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromRoute] string schemaName, [FromBody] CreateInventariAktualDto dto)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update([FromRoute] string schemaName, int id, [FromBody] UpdateInventariAktualDto dto)
        {
